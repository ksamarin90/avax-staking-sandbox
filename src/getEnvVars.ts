...
export const getEnvVars = <T extends string>(additionalEnvsKeys: T[] = []): ExampleEnvs<T> => {
	if (!(AVAX_PUBLIC_URL && P_CHAIN_ADDRESS && PRIVATE_KEY && X_CHAIN_ADDRESS && C_CHAIN_ADDRESS && CORETH_ADDRESS)) {
		throw new Error('Missing required environment variable(s). Please check your .env file.');
	}
